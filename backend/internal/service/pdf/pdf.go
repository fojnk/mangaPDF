...
func CreatePDF(chapterPath string, savedFiles []string) error {
	...
	for _, file := range savedFiles {
		...
		if width < height {
			pdf.AddPage()
			pdf.ImageOptions(imageFile, (models.PDFOpts.A4Width-width)/2, (models.PDFOpts.A4Height-height)/2, width, height, false, opt, 0, "")
		} else {
			pdf.AddPageFormat("L", pdf.GetPageSizeStr("A4"))
			pdf.ImageOptions(imageFile, (models.PDFOpts.A4Height-width)/2, (models.PDFOpts.A4Width-height)/2, width, height, false, opt, 0, "")
		}
	}
...
func resizeToFit(imgFilename string) (float64, float64, error) {
	...
	if width < height {
		widthScale = models.PDFOpts.MaxWidth / width
		heightScale = models.PDFOpts.MaxHeight / height
	} else {
		widthScale = models.PDFOpts.MaxHeight / width
		heightScale = models.PDFOpts.MaxWidth / height
	}
...
func pixelsToMM(val float64) float64 {
	return float64(val * models.PDFOpts.MmInInch / models.PDFOpts.DPI)
}
